...
public class RerankerService {
    ...
    private static final String RERANK_PROMPT_TEMPLATE = """
            请严格评估以下文档片段与用户问题的相关性。

            评分标准（务必严格遵守）：
            - 10分：文档内容直接回答了用户问题，包含精确匹配的答案
            - 7-9分：文档包含与问题强相关的内容，能部分回答问题
            - 4-6分：文档与问题有一定关联，属于同一领域但不直接相关
            - 1-3分：文档与问题几乎无关，仅有少量关键词重叠
            - 0分：完全无关，主题完全不匹配

            重要提示：
            - 如果文档主题与用户问题明显不符，请给出0-2分
            - 不要仅因关键词重叠就给出高分
            - 只保留真正相关的内容
