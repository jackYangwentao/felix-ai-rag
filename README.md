...
| 策略 | 核心思想 | 分块流程 | 特点 |
|------|----------|----------|------|
| **FixedSize** | 固定大小 | 1. 按分隔符分割<br>2. 累积段落直到接近 chunkSize<br>3. 超长段落强制切分<br>4. 保留重叠部分 | 简单快速，优先保持段落完整 |
| **Recursive** | 分层递归 | 1. 按优先级尝试分隔符（段落→句子→单词→字符）<br>2. 超长片段递归使用更细粒度分隔符<br>3. 合并时保留重叠 | 语义完整性好，适应不同文本 |
| **Semantic** | 语义断点 | 1. 分割为句子<br>2. 生成句子嵌入（带上下文缓冲）<br>3. 计算相邻句子余弦距离<br>4. 根据阈值识别语义跳跃点<br>5. 在跳跃点处分割 | 基于语义相似度，主题边界清晰 |
| **SentenceWindow** | 窗口扩展 | 1. 分割为句子<br>2. 每个句子作为中心<br>3. 前后各取 N 个句子构建窗口<br>4. 序列化为带标记的格式 | 小块索引高精度，检索后扩展为窗口 |
| **Markdown** | 结构保持 | 1. 提取代码块/表格（特殊处理）<br>2. 按标题层级分割<br>3. 合并小节时保持结构<br>4. 恢复特殊块 | 保持文档结构，代码块完整 |
...
| 策略 | 最佳适用场景 | 计算开销 | 检索精度 | 上下文完整性 |
|------|-------------|----------|----------|-------------|
| **FixedSize** | 简单文本、快速原型 | 低 | 中 | 中 |
| **Recursive** | 通用文本、多语言文档 | 低 | 中高 | 高 |
| **Semantic** | 主题敏感、语义边界重要 | 高（需嵌入计算） | 高 | 高 |
| **SentenceWindow** | 需要精确检索+丰富上下文 | 中 | 极高 | 极高 |
| **Markdown** | 技术文档、Markdown文件 | 低 | 高 | 高（结构） |

### 各策略详细说明

#### 1. 固定大小分块（FixedSize）

**原理**：按目标字符数切分，优先保持段落完整性

**流程**：
1. 使用分隔符（默认 `\n\n`）将文本分割为段落
2. 累积段落，直到接近 `chunkSize`
3. 如果单个段落超过 `chunkSize`，强制按固定大小切分
4. 保留 `chunkOverlap` 长度的重叠部分，保持上下文连续性
...
**流程**：
1. 尝试使用当前分隔符分割文本
2. 如果分割后的片段仍超过 `chunkSize`，递归使用更细粒度的分隔符
3. 合并相邻片段时保留重叠部分
...
**原理**：基于语义相似度检测断点，在主题变化处分割

**流程**：
1. 将文本分割为句子
2. 为每个句子生成嵌入向量（带上下文缓冲）
3. 计算相邻句子的余弦距离（1 - 余弦相似度）
4. 根据阈值类型计算断点阈值：
   - **Percentile**：使用第 N 百分位作为阈值
   - **Standard Deviation**：使用均值 + N 倍标准差
   - **Interquartile**：使用 Q3 + N * IQR
5. 在距离超过阈值的跳跃点处分割
...
rag:
  chunk:
    strategy: semantic
    semantic:
      threshold-type: percentile      # 阈值类型
      threshold-amount: 95.0          # 阈值数值
      buffer-size: 1                  # 上下文缓冲大小
```

#### 4. 句子窗口分块（SentenceWindow）
...
1. 将文本分割为句子
2. 每个句子作为中心句子
3. 前后各取 `windowSize` 个句子构建上下文窗口
4. 序列化为带标记的格式（`[BEFORE]...[CENTER]...[AFTER]`）

**特点**：
- 索引阶段：小块（单句）保证检索精度
- 检索阶段：用完整窗口替换单句，保证上下文丰富性
- 特别适合长文档问答
...
#### 5. Markdown结构分块（Markdown）

**原理**：基于 Markdown 文档结构进行智能分块

**流程**：
1. 提取特殊块（代码块、表格），用占位符替换
2. 按标题层级（`# ## ###`）分割文档
3. 合并小节时保持结构完整性
4. 恢复特殊块到对应位置
